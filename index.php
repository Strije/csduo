...
try {
	$db = new PDO('mysql:host=localhost;dbname=бд', 'логин', 'пароль', array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
...
switch ($_GET['page']) {
	...
	case 'support_new':
		...
		if($tid == 0) {
			...
			$row = $sql->fetch();
			$count = $row['COUNT(`id`)'];
			if($count != 0) exit(json_encode(array('success'=>false, 'error'=>'У вас уже есть тикет в ожидании поддержки.')));
			...
			exit(json_encode(array('success'=>true,'msg'=>'Спасибо - ваш тикет был отправлен ('.$id.')')));
		} else {
		...
		try
		{
			...
			if (!$openid->mode) {
				...
				if ($openid->validate()) {
					...
					$url = "http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=61559D134B3BF7FCED46D39B8E25861C&steamids=$matches[1]";
	...
	case 'confirm':
	if(!$user) exit(json_encode(array('success'=>false, 'error'=>'Вы должны войти в систему для доступа к подтверждения.')));
		$tid = (int)$_GET['tid'];
	...
	case 'get_bank_safe':
		...
		$g = curl('https://www.google.com/recaptcha/api/siteverify?secret=6Lf0RyITAAAAAO4UszReznIQpQ8dtdafkaCMl8re&response='.$_GET['g-recaptcha-response']);
